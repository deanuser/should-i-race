...
import 'piccolore';
import { g as renderComponent, r as renderTemplate, m as maybeRenderHead, e as addAttribute, h as Fragment } from './ssr-function_DFINGRnz.mjs';
import { g as getCollection, $ as $$Layout, a as $$Navbar, c as $$Footer, b as $$PartnerBanner } from './PartnerBanner_D2JhUW7f.mjs';
...
const $$SubRegionListing = createComponent(async ($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$props, $$slots);
  Astro2.self = $$SubRegionListing;
  const { country, region, subRegion } = Astro2.props;
  const allTracks = await getCollection("tracks");
  const tracks = allTracks.filter(
    (t) => t.data.country.toLowerCase() === country.toLowerCase() && t.data.region.toLowerCase() === region.toLowerCase() && t.data.subRegion.toLowerCase() === subRegion.toLowerCase()
  );
  if (subRegion === "all") {
    return Astro2.redirect(`/${country}/${region}`);
  }
  if (tracks.length === 0) {
    return Astro2.redirect("/404");
  }
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": `Tracks in ${subRegion?.replace(/-/g, " ")} | Should I Race`, "description": `Explore professional athletics tracks in ${subRegion?.replace(/-/g, " ")}` }, { "default": async ($$result2) => renderTemplate` ${renderComponent($$result2, "Navbar", $$Navbar, {})} ${maybeRenderHead()}<main class="container mx-auto px-4 py-12 mt-20 max-w-7xl animate-fade-in"> <!-- Breadcrumbs --> <nav class="flex text-sm text-slate-400 font-medium mb-8 bg-card-bg/50 border border-white/5 w-fit px-4 py-2 rounded-lg flex-wrap items-center gap-2"> <a href="/" class="hover:text-brightcyan transition-colors">Home</a> <span class="opacity-50">/</span> <a${addAttribute(`/${country}`, "href")} class="hover:text-brightcyan capitalize transition-colors">${country}</a> <span class="opacity-50">/</span> <a${addAttribute(`/${country}/${region}`, "href")} class="hover:text-brightcyan capitalize transition-colors">${region?.replace(/-/g, " ")}</a> <span class="opacity-50">/</span> <span class="text-white capitalize font-bold">${subRegion?.replace(/-/g, " ")}</span> </nav> <header class="mb-12"> <h1 class="text-5xl md:text-6xl font-black text-white mb-6 uppercase tracking-tight">Tracks in ${subRegion?.replace(/-/g, " ")}</h1> </header> <hr class="border-white/5 mb-10"> <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"> ${tracks.map((track) => renderTemplate`${renderComponent($$result2, "TrackCard", $$TrackCard, { "name": track.data.name, "city": track.data.city, "trackUrl": track.data.subRegion === "all" ? `/${track.data.country}/${track.data.region}/${track.data.slug}` : `/${track.data.country}/${track.data.region}/${track.data.subRegion}/${track.data.slug}` })}`)} </div> </main> ${renderComponent($$result2, "Footer", $$Footer, {})} ` })}`;
}, "/Users/deandavis/Documents/Work/Should I Run/astro/src/components/SubRegionListing.astro", void 0);

const $$slug = createComponent(async ($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$props, $$slots);
  Astro2.self = $$slug;
  const { country, region, slug } = Astro2.params;
  const tracks = await getCollection("tracks");
  const isSubRegion = tracks.some(
    (t) => t.data.country.toLowerCase() === country?.toLowerCase() && t.data.region.toLowerCase() === region?.toLowerCase() && t.data.subRegion.toLowerCase() === slug?.toLowerCase()
  );
  const track = tracks.find(
    (t) => t.data.slug === slug && t.data.country === country && t.data.region === region && t.data.subRegion === "all"
  );
  if (!track && !isSubRegion) {
    return Astro2.redirect("/404");
  }
  ...
  let forecasts = [];
  let dailyForecasts = [];
  let relativeWindDescription = "";
  let currentWindCategory = "mixed";
  let isMocked = false;
  if (track) {
    ...
    if (isMocked) {
      currentWind = { speed: shieldWindSpeed(3.2), deg: 210, desc: "scattered clouds", icon: "03d" };
      currentTemp = 14.5;
...
}, "/Users/deandavis/Documents/Work/Should I Run/astro/src/pages/[country]/[region]/[slug].astro", void 0);
const $$file = "/Users/deandavis/Documents/Work/Should I Run/astro/src/pages/[country]/[region]/[slug].astro";
const $$url = "/[country]/[region]/[slug]";
